...
import { CallType } from './-info.model.ts';
...
export const getLatestCall = () =>
  axios
    .get<CallType>('/contribution-calls/type/INFO_CIEFT/latest')
    .then(res => res.data);
...
function Calls() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: addCall,
    onError: () => toast.error('Nu s-a putut adăuga informația!'),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['calls'] });
      await queryClient.invalidateQueries({ queryKey: ['latestCall'] });
      setIsModalOpen(false);
      resetAllForm();
      toast.success('Informația a fost adăugat cu succes.');
    },
  });
  ...
  const { mutate: deleteCallMutation, isPending: isDeleteCallPending } =
    useMutation({
      ...
      onSuccess: async () => {
        ...
        toast.success('Informația a fost șters cu succes');
      },
  ...
  const { mutate: editCallMutation, isPending: isEditCallPending } =
    useMutation({
      ...
      onSuccess: async () => {
        ...
        toast.success('Informația a fost editat cu succes');
        resetEditCallForm();
  ...
  const showPropsConfirm = () => {
    confirm({
      title: 'Ștergere informație',
      icon: <ExclamationCircleFilled />,
      content: 'Sigur doriți să ștergeți informația ?',
      okText: 'Șterge',
      okType: 'danger',
      cancelText: 'Renunță',
      onOk() {
  ...
  const schema = yup.object().shape({
    ...
    publicationType: yup.string().default('INFO_CIEFT'),
  });
  ...
  return (
    <div>
      ...
      {isLoading || isFetching ? (
        ...
        <span>
          Informațiile nu pot fi afișate momentan. Reveniți mai târziu!
        </span>
      ) : isEmpty(data) ? (
        <div className="flex">
          <span>Nu există informații publicate momentan.</span>
        </div>
      ) : (
        <div>
          <KBanner label={`${data.title}`} />
      ...
      <Modal
        title="Creează un afiș cu informații pentru CIEFT"
        open={isModalOpen}
        ...
        <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <Input
                ...
                placeholder={
                  errors.title?.message ?? 'Titlu (se va afișa sus în antet) '
                }
                value={value}
                onChange={onChange}
                allowClear
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <Input
                status={errors.title ? 'error' : ''}
                placeholder={errors.title?.message ?? 'Ediția (an)'}
                value={value}
                onChange={onChange}
                allowClear
              />
      ...
      <Modal
        title="Schimbă pdf-ul informației"
