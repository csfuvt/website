...
const EditKMovingBanner: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<BannerImage[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  useEffect(() => {
    axios.get(BASE_URL + '/banner').then(res => setData(res.data));
  }, []);

  const isEditing = (record: BannerImage) => record.id === editingKey;

  // const edit = (record: Partial<BannerImage> & { id: number }) => {
  ...
  const save = async (id: number) => {
    try {
      const row = (await form.validateFields()) as BannerImage;
      ...
      if (index > -1) {
        ...
        await axios.put(`${BASE_URL}/banner/${id}`, row);
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    if (active.id !== over?.id) {
      ...
      try {
        ...
        await axios.post(`${BASE_URL}/banner/reorder`, {
  ...
  const columns = [
    {
      ...
      render: (url: string) => (
        <>
          <center>
            <Image src={`${BASE_URL}${url}`} alt="banner" width={200} />
          </center>
        </>
      ),
    },
    ...
    // },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: BannerImage) => {
...
export default EditKMovingBanner;
