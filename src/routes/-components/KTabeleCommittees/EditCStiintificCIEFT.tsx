...
interface ScientificCIEFTCommitteeMember {
  id: number;
  name: string;
  university: string;
  country: string;
...
// Componentă pentru un rând sortabil
const Row: React.FC<{
  'data-row-key': string;
...
const EditCStiintificCIEFT: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<ScientificCIEFTCommitteeMember[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  // Fetch data din backend
  useEffect(() => {
    axios.get('/committee/SCIENTIFIC_CIEFT').then(res => setData(res.data));
  }, []);

  const isEditing = (record: ScientificCIEFTCommitteeMember) =>
    record.id === editingKey;

  const edit = (
    record: Partial<ScientificCIEFTCommitteeMember> & { id: number }
  ) => {
  ...
  const save = async (id: number) => {
    try {
      const row =
        (await form.validateFields()) as ScientificCIEFTCommitteeMember;
  ...
  // Configurare senzori pentru drag-and-drop
  ...
  const columns = [
    ...
    },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: ScientificCIEFTCommitteeMember) => {
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...col,
      onCell: (record: ScientificCIEFTCommitteeMember) => ({
        record,
...
export default EditCStiintificCIEFT;
