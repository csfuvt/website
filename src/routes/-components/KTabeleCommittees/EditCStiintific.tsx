...
const EditCStiintific: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<ScientificCommitteeMember[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  // Fetch data din backend
  useEffect(() => {
    axios.get('/committee/SCIENTIFIC').then(res => setData(res.data));
  }, []);

  const isEditing = (record: ScientificCommitteeMember) =>
    record.id === editingKey;

  const edit = (
    record: Partial<ScientificCommitteeMember> & { id: number }
  ) => {
  ...
  const save = async (id: number) => {
    try {
      const row = (await form.validateFields()) as ScientificCommitteeMember;
  ...
  const columns = [
    ...
    },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: ScientificCommitteeMember) => {
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...col,
      onCell: (record: ScientificCommitteeMember) => ({
        record,
...
export default EditCStiintific;
