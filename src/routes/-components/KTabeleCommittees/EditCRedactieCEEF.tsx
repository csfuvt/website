...
interface EditorialCEEFCommitteeMember {
  id: number;
  name: string;
  email: string;
  role: string;
...
const EditCEditCRedactieCEEF: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<EditorialCEEFCommitteeMember[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  useEffect(() => {
    // Fetch initial data
    axios.get('/committee/EDITORIAL_CEEF').then(res => setData(res.data));
  }, []);

  const isEditing = (record: EditorialCEEFCommitteeMember) =>
    record.id === editingKey;

  const edit = (
    record: Partial<EditorialCEEFCommitteeMember> & { id: number }
  ) => {
  ...
  const save = async (id: number) => {
    try {
      const row = (await form.validateFields()) as EditorialCEEFCommitteeMember;
  ...
  const columns = [
    ...
    },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: EditorialCEEFCommitteeMember) => {
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...col,
      onCell: (record: EditorialCEEFCommitteeMember) => ({
        record,
...
export default EditCEditCRedactieCEEF;
