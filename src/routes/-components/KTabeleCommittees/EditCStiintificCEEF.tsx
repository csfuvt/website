...
interface ScientificCEEFCommitteeMember {
  id: number;
  name: string;
  university: string;
  country: string;
...
const EditCStiintificCEEF: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<ScientificCEEFCommitteeMember[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  // Fetch data din backend
  useEffect(() => {
    axios.get('/committee/SCIENTIFIC_CEEF').then(res => setData(res.data));
  }, []);

  const isEditing = (record: ScientificCEEFCommitteeMember) =>
    record.id === editingKey;

  const edit = (
    record: Partial<ScientificCEEFCommitteeMember> & { id: number }
  ) => {
  ...
  const save = async (id: number) => {
    try {
      const row =
        (await form.validateFields()) as ScientificCEEFCommitteeMember;
  ...
  const columns = [
    ...
    },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: ScientificCEEFCommitteeMember) => {
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...col,
      onCell: (record: ScientificCEEFCommitteeMember) => ({
        record,
...
export default EditCStiintificCEEF;
