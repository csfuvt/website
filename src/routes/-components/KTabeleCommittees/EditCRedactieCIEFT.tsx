...
interface EditorialCIEFTCommitteeMember {
  id: number;
  name: string;
  email: string;
  role: string;
...
const EditCEditCRedactieCIEFT: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<EditorialCIEFTCommitteeMember[]>([]);
  const [editingKey, setEditingKey] = useState<number | ''>('');

  useEffect(() => {
    // Fetch initial data
    axios.get('/committee/EDITORIAL_CIEFT').then(res => setData(res.data));
  }, []);

  const isEditing = (record: EditorialCIEFTCommitteeMember) =>
    record.id === editingKey;

  const edit = (
    record: Partial<EditorialCIEFTCommitteeMember> & { id: number }
  ) => {
  ...
  const save = async (id: number) => {
    try {
      const row =
        (await form.validateFields()) as EditorialCIEFTCommitteeMember;
  ...
  const columns = [
    ...
    },
    {
      title: 'Operațiuni',
      dataIndex: 'operation',
      render: (_: any, record: EditorialCIEFTCommitteeMember) => {
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...col,
      onCell: (record: EditorialCIEFTCommitteeMember) => ({
        record,
...
export default EditCEditCRedactieCIEFT;
